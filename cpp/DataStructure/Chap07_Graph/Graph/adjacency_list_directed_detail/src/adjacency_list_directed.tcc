		...
		const EdgeInfo & L_info)
{
	if(L_source >= this->M_vertex_size || L_target >= this->M_vertex_size) {
		std::throw_with_nested(std::out_of_range(std::string(
						"adjacency_list_directed error: "
						"add_edge() out of range.")));
	}//if

	typename std::list<std::pair<size_type, EdgeInfo> >::iterator L_find_iter =
		std::find_if(
...
template <typename EdgeInfo, typename VertexInfo, typename GraphInfo>
bool
adjacency_list_directed<EdgeInfo, VertexInfo, GraphInfo>::remove_edge(
		const size_type L_source,
		const size_type L_target)
{
	if(L_source >= this->M_vertex_size || L_target >= this->M_vertex_size) {
...
template <typename EdgeInfo, typename VertexInfo, typename GraphInfo>
typename adjacency_list_directed<EdgeInfo, VertexInfo, GraphInfo>::size_type
adjacency_list_directed<EdgeInfo, VertexInfo, GraphInfo>::add_vertex(const VertexInfo & L_info)
{
...
template <typename EdgeInfo, typename VertexInfo, typename GraphInfo>
bool
adjacency_list_directed<EdgeInfo, VertexInfo, GraphInfo>::remove_vertex(const size_type L_vertex_id)
{
...
adjacency_list_directed<EdgeInfo, VertexInfo, GraphInfo>::get_edge(
		const size_type L_source,
		const size_type L_target) const
{
	...
	typename std::list<std::pair<size_type, EdgeInfo> >::const_iterator L_find_iter =
		std::find_if(
			this->M_list[L_source].begin(),
			this->M_list[L_source].end(),
			[=](const std::pair<size_type, EdgeInfo> & rhs) {
...
template <typename EdgeInfo, typename VertexInfo, typename GraphInfo>
bool
adjacency_list_directed<EdgeInfo, VertexInfo, GraphInfo>::put_edge(
			const size_type L_source,
			const size_type L_target,
			const EdgeInfo & L_info)
{
...
template <typename EdgeInfo, typename VertexInfo, typename GraphInfo>
bool
adjacency_list_directed<EdgeInfo, VertexInfo, GraphInfo>::breadth_first_search_aux(
...
template <typename EdgeInfo, typename VertexInfo, typename GraphInfo>
bool
adjacency_list_directed<EdgeInfo, VertexInfo, GraphInfo>::depth_first_search_aux(
		const size_type L_start_vertex,
		size_type L_visited[],
		DFSVisitor<adjacency_list_directed> & L_vis)
{
	enum VISIT_STATE : size_type {
		UNVISITED = 0,
		VISITED,
		FINISHED
	};//VISIT_STATE

	L_visited[L_start_vertex] = VISIT_STATE::VISITED;
	if(!L_vis.discover_vertex(L_start_vertex, *this))
		return false;
		...
		++L_iter)
	{
		...
		if(L_visited[L_target] == VISIT_STATE::UNVISITED) {
			if(!L_vis.tree_edge(L_start_vertex, L_target, *this))
				return false;
			if(!depth_first_search_aux(L_target, L_visited, L_vis))
				return false;
		} else if(L_visited[L_target] == VISIT_STATE::VISITED) {
			if(!L_vis.back_edge(L_start_vertex, L_target, *this))
				return false;
		} else {//VISIT_STATE::FINISHED
			if(!L_vis.forward_or_cross_edge(L_start_vertex, L_target, *this))
				return false;
		}//if-else
	}//for

	L_visited[L_start_vertex] = VISIT_STATE::FINISHED;
