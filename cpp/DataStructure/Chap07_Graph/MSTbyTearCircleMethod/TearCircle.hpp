...
void MST(const AdjMatrixUndirNetwork<T, WT> & g) {
//std::vector<std::vector<WT>> MST(const AdjMatrixUndirNetwork<T, WT> & g) {
	Edge edge[100];
	int edge_num = 0;
	...
	for (int i = 0; i < edge_num; ++i) {
		g_temp.DeleteArc(edge[i].vertex1, edge[i].vertex2); ///-->13.vertex1 is illegal
	...
	g_temp.Display();
	//return g_temp.toVectorOfVector();
}
//#endif
