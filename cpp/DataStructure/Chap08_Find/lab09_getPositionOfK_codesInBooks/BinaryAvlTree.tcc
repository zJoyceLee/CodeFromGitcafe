...
template <typename T>
void DisplayBTWithTreeShapeHelp(const BinAVLTreeNode<T> * r, int level);
	//	按树状形式显示以r为根的平衡二叉树，level为层次数，可设根结点的层次数为1
template <typename T>
void DisplayBTWithTreeShape(const BinaryAVLTree<T> & bt);
	//	树状形式显示二叉平衡树bt

// 平衡二叉树类的实现部分
template <typename T>
BinaryAVLTree<T>::BinaryAVLTree()
// 操作结果：构造一个空平衡二叉树
{
	root = nullptr;
}
...
template <typename T>
BinAVLTreeNode<T> *BinaryAVLTree<T>::GetRoot() const
// 操作结果：返回平衡二叉树的根
{
	return root;
}
...
template <typename T>
bool BinaryAVLTree<T>::isEmpty() const
// 操作结果：判断平衡二叉树是否为空
{
	return root == nullptr;
}
...
template <typename T>
Status BinaryAVLTree<T>::GetElem(BinAVLTreeNode<T> *p, T & e) const
...
template <typename T>
Status BinaryAVLTree<T>::SetElem(BinAVLTreeNode<T> *p, const T & e)
...
template <typename T>
void BinaryAVLTree<T>::PreOrderHelp(BinAVLTreeNode<T> *r, void (*Visit)(const T & )) const
// 操作结果：先序遍历以r为根的平衡二叉树
{
	if (r != nullptr) 	{
		(*Visit)(r->data);					// 访问根结点
...
template <typename T>
void BinaryAVLTree<T>::PreOrder(void (*Visit)(const T & )) const
// 操作结果：先序遍历平衡二叉树
{
...
template <typename T>
void BinaryAVLTree<T>::InOrderHelp(BinAVLTreeNode<T> *r, void (*Visit)(const T & )) const
// 操作结果：中序遍历以r为根的平衡二叉树
{
	if (r != nullptr) 	{
...
template <typename T>
void BinaryAVLTree<T>::InOrder(void (*Visit)(const T & )) const
// 操作结果：中序遍历平衡二叉树
{
...
template <typename T>
void BinaryAVLTree<T>::PostOrderHelp(BinAVLTreeNode<T> *r, void (*Visit)(const T & )) const
// 操作结果：后序遍历以r为根的平衡二叉树
{
	if (r != nullptr) 	{
...
template <typename T>
void BinaryAVLTree<T>::PostOrder(void (*Visit)(const T & )) const
// 操作结果：后序遍历平衡二叉树
{
...
template <typename T>
void BinaryAVLTree<T>::LevelOrder(void (*Visit)(const T & )) const
// 操作结果：层次遍历平衡二叉树
{
	LinkQueue<BinAVLTreeNode<T> *> q;// 队列
	BinAVLTreeNode<T> *t = root;		// 从根结点开始进行层次遍历

	if (t != nullptr) q.EnQueue(t);			// 如果根非空,则入队
	while (!q.isEmpty())	{	            // q非空,说明还有结点未访问
		q.DelQueue(t);
		(*Visit)(t->data);
		if (t->leftChild != nullptr)			// 左孩子非空
			q.EnQueue(t->leftChild);		// 左孩子入队
		if (t->rightChild != nullptr)			// 右孩子非空
			q.EnQueue(t->rightChild);		// 右孩子入队
	}
}

template <typename T>
int BinaryAVLTree<T>::HeightHelp(const BinAVLTreeNode<T> *r) const
// 操作结果：求以r为根的平衡二叉树的高
{
	if(r == nullptr)	// 空平衡二叉树高为0
		return 0;
...
template <typename T>
int BinaryAVLTree<T>::Height() const
// 操作结果：求平衡二叉树的高
{
...
template <typename T>
BinaryAVLTree<T>::BinaryAVLTree(const T & e)
// 操作结果：建立以e为根的平衡二叉树
{
	root = new BinAVLTreeNode<T>(e);
}

template <typename T>
int BinaryAVLTree<T>::NodeCountHelp(const BinAVLTreeNode<T> *r) const
...
template <typename T>
int BinaryAVLTree<T>::NodeCount() const
// 操作结果：求平衡二叉树的结点个数
{
...
template <typename T>
BinAVLTreeNode<T> *BinaryAVLTree<T>::LeftChild(const BinAVLTreeNode<T> *p) const
// 操作结果：返回平衡二叉树结点p的左孩子
{
	return p->leftChild;
}
...
template <typename T>
BinAVLTreeNode<T> *BinaryAVLTree<T>::RightChild(const BinAVLTreeNode<T> *p) const
// 操作结果：返回平衡二叉树结点p的右孩子
{
	return p->rightChild;
}
...
template <typename T>
BinAVLTreeNode<T> * BinaryAVLTree<T>::ParentHelp(BinAVLTreeNode<T> *r, const BinAVLTreeNode<T> *p) const
// 操作结果：在以r为根的平衡二叉树中求结点p的双亲
{
	if (r == nullptr)
       return nullptr;		// 空平衡二叉树
	else if (r->leftChild == p || r->rightChild == p)
       return r;        // r为p的双亲
	else	{	        // 在子树上求双亲
		BinAVLTreeNode<T> *tmp;
...
template <typename T>
BinAVLTreeNode<T> *BinaryAVLTree<T>::Parent(const BinAVLTreeNode<T> *p) const
// 操作结果：返回平衡二叉树结点p的双亲
{
...
template <typename T>
BinAVLTreeNode<T> *BinaryAVLTree<T>::Find(const T & key, BinAVLTreeNode<T> *& f) const
// 操作结果: 返回指向关键字为key的数据元素的指针,并用f返回其双亲
{
	BinAVLTreeNode< T> *p = GetRoot(); // 指向当前结点
	f = nullptr;								  // 指向p的双亲

	while (p != nullptr &&  p->data != key)	{	  // 查找关键字为key的结点
...
template <typename T>
BinAVLTreeNode<T> *BinaryAVLTree<T>::Find(const T & key)	const
// 操作结果: 返回指向关键字为key的数据元素的指针
{
	BinAVLTreeNode<T> *f;			// 指向被查找结点的双亲
	return Find(key, f);
}

template <typename T>
BinAVLTreeNode<T> *BinaryAVLTree<T>::Find(
	const T & key,	BinAVLTreeNode<T> *& f,
...
// 查找关键字为key的数据元素,返回指向关键字为key的元素的指针,并用f返回其双亲,栈s存储查找路径.
{
	...
	f = nullptr;		                        // 指向p的双亲
	s.Clear();
	while (p != nullptr &&  p->data != key)	{	// 查寻关键字为key的结点
		if (key < p->data)	{	            // key更小,在左子树上进行查找
			f = p;
			s.Push(p);
			p = p->leftChild;
...
template <typename T>
void BinaryAVLTree<T>::LeftRotate(BinAVLTreeNode<T> *& subRoot)
// 操作结果: 对以subRoot为根的二叉树作左旋处理,处理之后subRoot指向新的树根结点,
//	也就是旋转处理前的右子树的根结点
{
	BinAVLTreeNode<T> *ptrRChild;
...
template <typename T>
void BinaryAVLTree<T>::RightRotate(BinAVLTreeNode<T> *& subRoot)
// 操作结果: 对以subRoot为根的二叉树作右旋处理,处理之后subRoot指向新的树根结点,
//	也就是旋转处理前的左子树的根结点
{
	BinAVLTreeNode<T> *pLChild;
...
template <typename T>
bool BinaryAVLTree<T>::Insert(const T & e)
// 操作结果: 插入数据元素e
{
	BinAVLTreeNode<T> *f, *af, *a, *b, *c;
	if (Find(e, f, a, af) == nullptr)	{	    // 查找失败, 则插入成功
		BinAVLTreeNode<T> *p;	    // 插入的新结点
		p = new BinAVLTreeNode<T>(e);// 生成插入结点
		p->bf = 0;
...
template <typename T>
void BinaryAVLTree<T>::DeleteLeftBalance(BinAVLTreeNode<T> *&
subRoot, bool & isShorter)
// 操作结果: 对以subRoot为根的二叉树删除时作左平衡处理, 删除subRoot的左子树上的结点，处
//	理后subRoot指向新的树根结点
{
	BinAVLTreeNode<T> *ptrRChild, *ptrRLChild;
	ptrRChild  = subRoot->rightChild;	// ptrRChild指向subRoot右孩子
...
template <typename T>
void BinaryAVLTree<T>::DeleteRightBalance(BinAVLTreeNode<T> *
& subRoot, bool & isShorter)
// 操作结果: 对以subRoot为根的二叉树删除时作右平衡处理, 删除subRoot的右子树上的结点，处
//	理后subRoot指向新的树根结点
{
	BinAVLTreeNode<T> *ptrLChild, *ptrLRChild;
...
template <typename T>
void BinaryAVLTree<T>::DeleteBalance(const T & key,
LinkStack<BinAVLTreeNode<T> *> & s)
// 操作结果: 根据查找路径，从删除结点进行回溯,并作平衡处理
{
	bool isShorter = true;
	while (!s.isEmpty() &&  isShorter)	{
		BinAVLTreeNode<T> *ptrCurNode, *ptrParent;
...
template <typename T>
void BinaryAVLTree<T>::DeleteHelp(const T & key,
BinAVLTreeNode<T> *& p, LinkStack< BinAVLTreeNode<T> *> & s)
// 操作结果: 删除p指向的结点
{
	BinAVLTreeNode<T> *tmpPtr, *tmpF;
	if (p->leftChild == nullptr &&  p->rightChild == nullptr)	{// p为叶结点
		delete p;
		p = nullptr;
		DeleteBalance(key, s);
	}
	else if (p->leftChild == nullptr)	{	// p只有左子树为空
		tmpPtr = p;
		p = p->rightChild;
		delete tmpPtr;
		DeleteBalance(key, s);
	}
	else if (p->rightChild == nullptr)	{	// p只有右子树非空
		tmpPtr = p;
		p = p->leftChild;
		delete tmpPtr;
		DeleteBalance(key, s);
		...
		s.Push(tmpF);
		tmpPtr = p->leftChild;
		while (tmpPtr->rightChild != nullptr)	{// 查找p在中序序列中直接前驱tmpPtr及其双亲tmpF
			tmpF = tmpPtr;
			s.Push(tmpF);
...
template <typename T>
bool BinaryAVLTree<T>::Delete(const T & key)
// 操作结果: 删除关键字为key的结点
{
	BinAVLTreeNode<T> *p, *f;
	LinkStack< BinAVLTreeNode<T> *> s;
	p = Find(key, f, s);
	if ( p == nullptr)	    // 查找失败, 则删除失败
		return false;
	else	{	        // 查找成功, 插入失败
		if (f == nullptr)	// 被删除结点为根结点
			DeleteHelp(key, p, s);
...
template <typename T>
void BinaryAVLTree<T>::DestroyHelp(BinAVLTreeNode<T> *& r)
// 操作结果：销毁以r的平衡二叉树
{
	if(r != nullptr)	{	// r非空,实施销毁
...
template <typename T>
BinAVLTreeNode<T> *BinaryAVLTree<T>::CopyTreeHelp(BinAVLTreeNode<T> *copy)
// 操作结果：将以copy为根的平衡二叉树复制成新的平衡二叉树,返回新平衡二叉树的根
{
	if (copy == nullptr)	// 复制空平衡二叉树
		return nullptr;					// 空平衡二叉树根为空
	else	{	// 复制非空平衡二叉树
		BinAVLTreeNode<T> *lChild = CopyTreeHelp(copy->leftChild);	// 复制左子树
		BinAVLTreeNode<T> *rChild = CopyTreeHelp(copy->rightChild);	// 复制右子树
		BinAVLTreeNode<T> *r = new BinAVLTreeNode<T>(copy->data, copy->bf, lChild, rChild);
			// 复制根结点
		return r;
	}
}
...
template <typename T>
BinaryAVLTree<T>::BinaryAVLTree(const BinaryAVLTree<T> & copy)
...
template <typename T>
BinaryAVLTree<T>::BinaryAVLTree(BinAVLTreeNode<T> *r)
// 操作结果：建立以r为根的平衡二叉树
{
	root = r;	// 复制平衡二叉树
}
...
template <typename T>
BinaryAVLTree<T> & BinaryAVLTree<T>::operator=(const BinaryAVLTree<T> & copy)
...
//	操作结果：按树状形式显示以r为根的平衡二叉树，level为层次数，可设根结点的层次数为1
{
	if(r != nullptr)	{	//空树不显式，只显式非空树
		DisplayBTWithTreeShapeHelp<T>(r->rightChild, level + 1);//显示右子树
		std::cout << std::endl << std::endl;					//显示新行
		for(int i = 0; i < level - 1; i++)
			std::cout << "       ";				//确保在第level列显示结点
		std::cout << r->data << "(" << r->bf << ")";				//显示结点
		DisplayBTWithTreeShapeHelp<T>(r->leftChild, level + 1);//显示左子树
	}
}
